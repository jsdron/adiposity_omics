...
# maf_filter=0.01
#
# zcat ../result/all/${trait}/ukb.${trait}.merged.chr1-chrX.regenie.gz | awk -F "\t" -v x="$maf_filter" '{ if(($12 >= x)) { print } }' | cut --complement -f6,7,8,10,11,15,16,17,18,19,20,21 | sed '1s/.*/Name\tChr\tPos\tRef\tAlt\tEffect\tPval\tAAF\tN\tSE\tINFO\tMAC/' | sed -e 's:;:\t:g' -e 's:REGENIE_SE=::g' -e 's:INFO=::g' -e 's:MAC=::g' | gzip > $output/ukb.${trait}.merged.chr1-chrX.reduced.regenie.gz
...
zcat ../result/all/${trait}/ukb.${trait}.merged.chr1-chrX.regenie.gz | cut --complement -f6,7,8,17,18,19,20,21 | sed '1s/.*/Name\tChr\tPos\tRef\tAlt\tEffect\tLCI_Effect\tUCI_Effect\tPval\tAAF\tN\tN_hom_ref\tN_het\tSE\tINFO\tMAC/' | sed -e 's:;:\t:g' -e 's:REGENIE_SE=::g' -e 's:INFO=::g' -e 's:MAC=::g' | gzip > $output/ukb.${trait}.merged.chr1-chrX.reduced.regenie.gz

rm ../result/all/${trait}/ukb.${trait}.merged.chr1-chrX.regenie.gz
